defmodule StrawHat.Review.CommentReactionsTest do
  ...
  describe "find_comments_reactions/1" do
    test "with valid id" do
      ...
      assert {:ok, _comments_reactions} =
               CommentReactions.find_comments_reactions(comments_reactions.id)
    end

    test "with invalid id shouldn't find the comments_reactions" do
      assert {:error, _reason} = CommentReactions.find_comments_reactions(8347)
    end
  end

  test "get_comments_reactions/1 list the comments_reactions per page" do
    insert_list(10, :comments_reactions)
    comments_reactions = CommentReactions.get_comments_reactions(%{page: 2, page_size: 5})
  ...
  test "create_comments_reactions/1 with valid inputs creates a comments_reactions" do
    ...
    assert {:ok, _comments_reactions} = CommentReactions.create_comments_reactions(params)
  end
  ...
  test "update_comments_reactions/2 with valid inputs updates a comments_reactions" do
    ...
    {:ok, comments_reactions} =
      CommentReactions.update_comments_reactions(comments_reactions, %{user_id: "user:546"})
  ...
  test "destroy_comments_reactions/1 with a found review destroys the comments_reactions" do
    ...
    assert {:ok, _} = CommentReactions.destroy_comments_reactions(comments_reactions)
  end
end
